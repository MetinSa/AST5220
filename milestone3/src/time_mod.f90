...
contains
  ...
  subroutine initialize_time_mod
    ...
    integer(i4b) :: i, n
    real(dp)     :: dx, dx_eta, x_eta1, x_eta2, x_infl, a_init, a_end, eta_init, yp1, yp2, epsilon, steplength, steplength_min
    ...
    n1          = 1000                      ! Number of grid points prior to recombination
    ...
    ! Inlcuding pre recombination times

    do i = 1, n1
       x_t(i) = x_infl + (i-1)*((x_start_rec - x_infl)/(n1-1))
    end do

    ! Filling in x-grid during recombination
    do i = 1, n2 
       x_t(n1+i) = x_start_rec + i*((x_end_rec - x_start_rec)/(n2))
    end do

    ! Filling in x-grid post recombination til today
    do i = 1, n3

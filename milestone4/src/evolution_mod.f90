 module evolution_mod
  ...
  real(dp),       allocatable, dimension(:) :: k_plot

  ! Source function parameters ( Milestone 4 specific)
  integer(i4b),                   parameter :: n_k_highres   = 5000
  integer(i4b),                   parameter :: n_x_highres   = 5000
  real(dp), allocatable, dimension(:,:)     :: S_lowres
...
contains
  ...
  subroutine get_hires_source_function(x_highres, k_highres, S)
    implicit none

    ! ---------------------------------------------------------------
    ! Function which pre-computes a 2D array over k and x for the 
    ! source function S(k,x).
    ! ---------------------------------------------------------------

    ! Arrays
    real(dp), pointer, dimension(:),   intent(out) :: x_highres, k_highres
    real(dp), pointer, dimension(:,:), intent(out) :: S

    ! Parameters
    integer(i4b) :: i, k 
    real(dp)     :: g, dg, ddg, tau, dtau, ddtau, H_p, dH_p, ddHH_p, Pi, dPi, ddPi, d_H_pdH_p
    real(dp)     :: ck, x, a
    
    ! Allocating arrays require to compute both a high and a low resolution S(k,x)
    allocate(k_highres(n_k_highres))
    allocate(x_highres(n_x_highres))
    allocate(S_lowres(n_t,n_k))   !n_t is length of x_t array
    allocate(S(n_x_highres, n_k_highres))
    allocate(S_coeff(4, 4, n_t, n_k))

    ! Filling k_source and x_source grids linearly
    do i = 1, n_k_highres

       k_highres(i) = ks(1) + (ks(n_k) - ks(1))*((i - 1.d0)/(n_k_highres - 1.d0))
       x_highres(i) = x_t(1) + (x_t(n_t) - x_t(1))*((i - 1.d0)/(n_x_highres - 1.d0))

    end do

    
    ! Computing the low resolution source function using grids x_t and n_t from
    ! previous milestones. 
    ...
    do k = 1, n_k
       ...
       do i = 1, n_t
          ...
          ! Substitutions to be used in the expression for the source function
          x = x_t(i)
          a = exp(x)
          g      = get_g(x)
          dg     = get_dg(x)
          ...
          H_p    = get_H_p(x)
          dH_p   = get_dH_p(x)
          d_H_pdH_p = 2.d0*H_p*dH_p + H_0**2*(3.d0*(Omega_b + Omega_m) + 8.d0*(Omega_r + Omega_nu)/a)/(2.d0*a)


          ! Computing the Pi terms and their derivatives
          Pi = Theta(i,2,k)
          dPi = dTheta(i,2,k)
          ddPi = (ck/(5.d0*H_p))*(2.d0*(dTheta(i,1,k) - Theta(i,1,k)*dH_p/H_p) - 3.d0*(dTheta(i,3,k) &
                 - Theta(i,3,k)*dH_p/H_p)) + (ddtau*Pi + dtau*dPi)*(9.d0/10.d0)

          ! Computing the low resolution source function 
          S_lowres(i,k) = g*(Theta(i,0,k) + Psi(i,k) + Pi/4.d0) + exp(-tau)*(dPsi(i,k) - dPhi(i,k)) &
                          - (dH_p*g*v_b(i,k) + H_p*g*dv_b(i,k) + H_p*dg*v_b(i,k))/(ck) &
                          + 3.d0*(d_H_pdH_p*g*Pi + 3.d0*H_p*dH_p*(dg*Pi + g*dPi) &
                          + H_p**2*(ddg*Pi + 2.d0*dg*dPi + g*ddPi))/(4.d0*ck**2)
       end do

    end do

    ! Computing a 2D spine through the S_lowres function using splin2_full_precom function from spline_2D.mod.f90
    call splie2_full_precomp(x_t, ks, S_lowres, S_coeff)

    ! Resampeling the low resolution source function to a high resolution 5000x5000 grid
    do k = 1, n_k_highres

       do i = 1, n_x_highres
          
          S(i,k) = splin2_full_precomp(x_t, ks, S_coeff, x_highres(i), k_highres(k))
  ...
  subroutine initialize_perturbation_eqns
    ...
    ! Initializing arrays to have 0 instead of random memory
    Theta(:,:,:)    = 0.d0
    dTheta(:,:,:)   = 0.d0
    dPhi(:,:)       = 0.d0
    dPsi(:,:)       = 0.d0
  ...
  subroutine integrate_perturbation_eqns
    ...
    do k = 1, n_k
       ...
       do i = i_tc, n_t
          ...
          dPhi(i,k)     = derivative(5)
          dv_b(i,k)     = derivative(4)
          ...
          dPsi(i,k)     = -dPhi(i,k) - (12.d0*H_0**2.d0)/(ck*a)**2.d0 * Omega_r*(-2.d0*Theta(i,2,k)+dTheta(i,2,k))
    ...
    open(15, file = 'dtheta.unf', form = 'unformatted', action = 'write', status = 'replace')
    write(15) dTheta
    write(15) x_t
    close(15)

    open(16, file = 'dpsi.unf', form = 'unformatted', action = 'write', status = 'replace')
    write(16) dphi
    write(16) dpsi
    write(16) x_t
    close(16)
  ...
  subroutine dydx(x, y, derivative)
   ...
   ! Theta2        = -((20.d0*ckH_p)/(45.d0*dtau))*Theta1
    Psi           = -Phi - 12.d0*Omega_r*Theta2*(H_0/(c*k_current*a))**2
    dPhi          = Psi - ((ckH_p**2)/3.d0)*Phi + (((H_0/H_p)**2)/2.d0)*( ((Omega_m*delta)/a) + ((Omega_b*delta_b)/a) + ((4.d0*Omega_r*Theta0)/a**2.d0) )
    ddelta        = ckH_p*v - 3.d0*dPhi
    ddelta_b      = ckH_p*v_b - 3.d0*dPhi
    dv            = -v - ckH_p*Psi
    ...
    do l = 9, 11
       derivative(l) = ((l-6.d0)*ckH_p*y(l-1))/(2.d0*(l-6.d0) + 1.d0) - (((l-6.d0)+1.d0)*ckH_p*y(l+1))/(2.d0*(l-6.d0) + 1.d0) + dtau*y(l)
    end do
    
    derivative(12) = ckH_p*y(5+lmax_int) - (c*(lmax_int +1.d0)*y(6+lmax_int))/(H_p*get_eta(x)) + dtau*y(6+lmax_int)
